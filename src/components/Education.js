...
const Details = ({ type, time, place, info, gpa, honors}) => {
  ...
  return (
    <li ref={ref} className="my-8 first:mt-0 last:mb-0 w-[60%] mx-auto flex flex-col items-center justify-between max-md:w-[80%]">
      ...
      >
        <h3 className="capitalize font-bold text-2xl max-sm:text-xl max-xs:text-lg">
          {type}
        </h3>
        <span className="capitalize font-medium text-dark/75 dark:text-light/75 max-xs:text-sm">
          {time} | {place}
        </span>
